...
Secondly, the synthesizing of a `compile :: Diag` is a little bit more involved since its `pic` can not be synthesized by only concatenating its childern it also requires a third `Diag`, the result of the compile. Since this third `Diag` is not one of the children we need to synthesize it by calling the code generated by UUAGC directly.
